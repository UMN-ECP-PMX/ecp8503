...
[^3]: MeRGE Expo 1 <https://merge.metrumrg.com/expo/expo1-nonmem-foce/>
...
-   Normal scale: $RSE=\frac{SE}{\hat{\theta}} \times 100\%$[^4]
...
[^4]: $\hat{\theta}$: point estimate of a parameter
...
-   If a logit-normal distribution is used:
    ...
    -   $CV\%$ is not appropriate, $SD$ is calculated instead using exact moments[^5].

[^5]: https://github.com/metrumresearchgroup/pmparams/blob/main/R/getSD-logit0.R
...
-   Example: $Corr_{(\omega^2_{CL/F},\omega^2_{V2/F})}=\frac{0.0703}{sqrt(0.114) \times sqrt(0.0824)}=0.725$
:::
:::
...
::: columns
::: {.column width="50%"}
-   When the individual data brings only **few information (i.e., sparse)** about the individual parameter value, the individual estimates (e.g., empirical Bayes estimates, EBEs) is close to (or “shrinks” to) population mean[^6].
-   Output file:`.shk`
...
[^6]: Understanding shrinkage <https://monolixsuite.slp-software.com/monolix/2024R1/understanding-shrinkage>

## $\eta$ shrinkage[^7] {style="font-size: 1em"}

[^7]: Savic RM, Karlsson MO. *AAPS J*. 2009
...
[^8]: Savic RM, Karlsson MO. *AAPS J*. 2009
