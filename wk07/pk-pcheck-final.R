...
options(mrggsave.dir = here("wk07/assets"), mrg.script = "pk-pcheck-final.R")
...
#' argument
# data <- nm_join(here("model/106"), .superset = TRUE)
...
data <- left_join(b,c) %>% left_join(a) %>% filter(is.na(C))


spec <- ys_load(here("wk07/data/analysis3.yml"))
...
mod <- mread("106.txt", project = here("wk07/model"))
...
mrggsave_last(tag = "auc-pcheck", height = 6, width = 8, labeller = NULL)
...
mrggsave_last(tag = "cmin-pcheck", height = 6, width = 7, labeller = NULL)
