...
-   Be careful. You can add COV effect without nesting[^4].

[^4]: Richard Brundage. Model Building.
...
Full fixed-effect modeling (FFEM)[^5] procedure:
...
Incorporate covariate effects in the model based on[^6]:

[^6]: Marc Gastonguay. *PAGE*. 2004.
...
For a general covariate with $\geq1$ degree of freedom ($df_i$)[^7]:

[^7]: df: degrees of freedom of a covariate; GVIF: generalized VIF (raw, not directly comparable across covariates with different df); aGVIF: adjusted GVIF (comparable across covariate with different df)
...
## Construct FFEM[^8] {style="font-size: 1em"}

[^8]: Marc Gastonguay. *PAGE*. 2004.
...
## Pros and Cons of selection methods[^9] {style="font-size: 1em"}

[^9]: Niclas Jonsson and Joakim Nyberg. *ACoP*. 2025.
...
## Pros and Cons of Pre-defined methods[^10]

[^10]: Marc Gastonguay. *PAGE*. 2004.
