...
-   Follow the [tutorial](https://msi.umn.edu/our-resources/knowledge-base/interactive-connections-faqs/how-do-i-setup-ssh-keys) specified by MSI to setup SSH key in your local computer.
    ...
    -   Create `~/.ssh/config` locally.
        ...
        -   Windows user (required):
            ...
            -   In terminal, type `mv config.txt config`
            -   If you do `ssh agate.msi.umn.edu`, it should ask you for passphrase now instead of password!
...
## Windows user ONLY: `dos2unix`

-   Windows and Linux use different “end-of-line” characters in text files.
-   When you create a SLURM `.sh` script on Windows and submit it to a Linux-based HPC (e.g. MSI), the file may contain hidden Windows carriage-return characters `\r`. Linux may then misread parts of the script.
-   Therefore, Windows users need one additional step to reformat `.sh` script created/edited on Windows using `dos2unix` tool before job submission: 
    -   Open a terminal/powershell. Run `winget install waterlan.dos2unix` (First time ONLY)
    -   Close the terminal/powershell, open a new terminal/powershell: 
        -   `cd your/ecp8503/local/repository`
        -   `dos2unix runmod.sh`
-   This is not an issue for macOS users. 
