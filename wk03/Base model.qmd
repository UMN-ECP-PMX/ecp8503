...
-   In NONMEM, to specify a MODEL for BSV, we write it in terms of ETA, and say OMEGA ($\omega$) is the standard deviation of the ETAs ($\omega^2$ is variance).
    -   $CL = TVCL + ETA(1)$
    -   $V = TVV*EXP(ETA(2))$
...
-   In NONMEM, to specify a MODEL for RUV, we write it in terms of EPS/ERR, and say SIGMA ($\sigma$) is the standard deviation of the EPS/ERR ($\sigma^2$ is variance).

-   One EPS value per observation.
...
-   Can have more than one `$OMEGA`
-   Can incorporate off-diagonal elements (block matrices) to characterize covariance/correlations among BSVs.

``` bash
$OMEGA BLOCK(2) ; VARIANCE COVARIANCE BLOCK(2)
 0.64           ; CL
 -0.24 0.58     ; V
